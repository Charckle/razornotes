...
def setup_email_reminder_scheduler(scheduler: APScheduler, app):
            ...
            for user in users:
                if user.get('email'):
                    ...
                    if game_url:
                        ...
                        if success:
                            app.logger.info(f"Morning reminder sent to {user['email']}")
                        else:
                            app.logger.error(f"Failed to send morning reminder to {user['email']}")
            ...
            for user in users:
                if user.get('email'):
                    ...
                    if game_url:
                        ...
                        if success:
                            app.logger.info(f"Evening reminder sent to {user['email']}")
                        else:
                            app.logger.error(f"Failed to send evening reminder to {user['email']}")
            ...
            if birthdays:
                ...
                for user in users:
                    if user.get('email'):
                        ...
                        if success:
                            app.logger.info(f"Monthly birthday reminder sent to {user['email']}")
                        else:
                            app.logger.error(f"Failed to send monthly birthday reminder to {user['email']}")
            ...
            if birthdays:
                ...
                for user in users:
                    if user.get('email'):
                        ...
                        if success:
                            app.logger.info(f"Daily birthday reminder sent to {user['email']}")
                        else:
                            app.logger.error(f"Failed to send daily birthday reminder to {user['email']}")
